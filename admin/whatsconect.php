...
// Auto-migração
$conn->query("ALTER TABLE configs ADD COLUMN IF NOT EXISTS evo_apiurl VARCHAR(255) DEFAULT ''");
$conn->query("ALTER TABLE configs ADD COLUMN IF NOT EXISTS evo_token  VARCHAR(255) DEFAULT ''");
...
function curlEvo($url, $token, $method = 'GET', $body = null, $timeout = 15) {
    ...
    curl_close($ch);
    return ['body'=>$resp,'errno'=>$errno,'error'=>$error,'code'=>$code,'ms'=>round($time*1000)];
}
...
if (isset($_GET['ajax'])) {
    ...
    $tok       = $api_admin['token'] ?? '';
    if (!empty($api_base) && !preg_match('#^https?://#i', $api_base)) $api_base = 'http://'.$api_base;

    $wpp  = carregarWpp($conn, $byid);
    $inst = trim($wpp['sessao'] ?? '');
    $acao = $_GET['ajax'];

    // ── SALVAR API GLOBAL ──
    if ($acao === 'salvar_api') {
        $url_s = esc_s($_POST['evo_apiurl'] ?? '');
        $tok_s = esc_s($_POST['evo_token']  ?? '');
        if (empty($url_s) || empty($tok_s)) { echo json_encode(['ok'=>false,'msg'=>'Preencha URL e Token!']); exit; }
        $ok = $conn->query("UPDATE configs SET evo_apiurl='$url_s', evo_token='$tok_s' WHERE id=1");
        echo json_encode(['ok'=>(bool)$ok,'msg'=>$ok?'✅ API Global salva! Todos os revendedores já usam este servidor.':'Erro: '.$conn->error]);
        exit;
    }

    // ── TESTAR API GLOBAL ──
    if ($acao === 'testar_api') {
        $url_t = trim($_POST['evo_apiurl'] ?? $api_base);
        $tok_t = trim($_POST['evo_token']  ?? $tok);
        if (!preg_match('#^https?://#i', $url_t)) $url_t = 'http://'.$url_t;
        $r = curlEvo(rtrim($url_t,'/').'/instance/fetchInstances', $tok_t, 'GET', null, 10);
        if ($r['errno'] !== 0) { echo json_encode(['ok'=>false,'msg'=>'Erro cURL: '.$r['error']]); exit; }
        $d = json_decode($r['body'], true);
        if ($r['code'] === 200 && is_array($d))
            echo json_encode(['ok'=>true,'msg'=>'✅ Conexão OK! '.count($d).' instância(s). ('.$r['ms'].'ms)']);
        elseif ($r['code'] === 401 || $r['code'] === 403)
            echo json_encode(['ok'=>false,'msg'=>'❌ Token inválido! HTTP '.$r['code']]);
        else
            echo json_encode(['ok'=>false,'msg'=>'HTTP '.$r['code'].': '.mb_substr($r['body'],0,200)]);
        exit;
    }

    // ── STATUS ──
    if ($acao === 'status') {
        if (empty($api_base)||empty($tok)||empty($inst)) { echo json_encode(['state'=>'not_configured']); exit; }
        $r = curlEvo($api_base.'/instance/connectionState/'.urlencode($inst), $tok, 'GET', null, 8);
        $d = json_decode($r['body'], true);
        if ($d && !isset($d['state']) && isset($d['instance']['state'])) $d['state'] = $d['instance']['state'];
        echo $d ? json_encode($d) : json_encode(['state'=>'error']);
        exit;
    }
    ...
    if ($acao === 'criar') {
        ...
        $tb   = intval($_POST['target_byid'] ?? $byid);
        if (empty($nome)||empty($api_base)||empty($tok)) { echo json_encode(['erro'=>'API não configurada.']); exit; }
        ...
        if ($r['errno'] !== 0) { echo json_encode(['erro'=>'cURL: '.$r['error']]); exit; }
        $d  = json_decode($r['body'], true);
        $criou = isset($d['instance']['instanceName']) || isset($d['instanceName']);
        $ja    = stripos($r['body'],'exist') !== false;
        if ($criou || $ja) {
            $ns  = esc_s($nome);
            $chk = $conn->query("SELECT id FROM whatsapp WHERE byid='$tb'");
            if ($chk && $chk->num_rows > 0) $conn->query("UPDATE whatsapp SET sessao='$ns' WHERE byid='$tb'");
            else $conn->query("INSERT INTO whatsapp (byid,sessao,apiurl,token,ativo) VALUES ('$tb','$ns','','','1')");
            echo json_encode(['ok'=>true,'msg'=>$ja?'Instância vinculada!':'Instância "'.$nome.'" criada!']);
        } else {
            echo json_encode(['erro'=>($d['message']??($d['error']??mb_substr($r['body'],0,200)))]);
        }
        exit;
    }
    ...
    // ── QR ──
    if ($acao === 'qr') {
        if (empty($api_base)||empty($tok)||empty($inst)) { echo json_encode(['erro'=>'Configure a instância primeiro.']); exit; }
        $rs = curlEvo($api_base.'/instance/connectionState/'.urlencode($inst), $tok, 'GET', null, 8);
        $ds = json_decode($rs['body'], true);
        $st = $ds['instance']['state'] ?? $ds['state'] ?? '';
        if ($st === 'open') { echo json_encode(['state'=>'open']); exit; }
        $r = curlEvo($api_base.'/instance/connect/'.urlencode($inst), $tok, 'GET', null, 20);
        $d = json_decode($r['body'], true);
        echo $d ? $r['body'] : json_encode(['erro'=>'Resposta inválida']);
        exit;
    }

    // ── LOGOUT ──
    if ($acao === 'logout') {
        if (empty($api_base)||empty($tok)||empty($inst)) { echo json_encode(['ok'=>false,'msg'=>'Instância não configurada.']); exit; }
        $r = curlEvo($api_base.'/instance/logout/'.urlencode($inst), $tok, 'DELETE', null, 12);
        $d = json_decode($r['body'], true);
        echo $d ? $r['body'] : json_encode(['ok'=>true]);
        exit;
    }
    ...
    if ($acao === 'deletar_inst') {
        $tb = intval($_POST['target_byid'] ?? $byid);
        $wt = carregarWpp($conn, $tb);
        $it = trim($wt['sessao'] ?? '');
        if (empty($api_base)||empty($tok)) { echo json_encode(['ok'=>false,'msg'=>'API não configurada.']); exit; }
        if (empty($it)) {
            $conn->query("UPDATE whatsapp SET sessao='' WHERE byid='$tb'");
            echo json_encode(['ok'=>true,'msg'=>'Banco limpo!']); exit;
        }
        curlEvo($api_base.'/instance/logout/'.urlencode($it), $tok, 'DELETE', null, 6);
        sleep(1);
        $r = curlEvo($api_base.'/instance/delete/'.urlencode($it), $tok, 'DELETE', null, 15);
    ...
    if ($acao === 'testar') {
        ...
        $ok = ($r['code'] >= 200 && $r['code'] < 300 && $r['errno'] === 0) &&
              (isset($d['key'])||isset($d['id'])||isset($d['messageId'])||(isset($d['status'])&&!in_array(strtolower($d['status']),['error','failed'])));
        echo json_encode(['ok'=>$ok,'msg'=>$ok?'✅ Enviado para '.$num.'!':'Falha HTTP '.$r['code'].': '.($d['message']??$d['error']??mb_substr($r['body'],0,200))]);
        exit;
    }
    ...
    if ($acao === 'get_msg') {
        $id = intval($_GET['id'] ?? 0);
        $tb = intval($_GET['byid'] ?? $byid);
        $res = $conn->query("SELECT * FROM mensagens WHERE id='$id' AND byid='$tb' LIMIT 1");
    ...
    // ── LISTAR INSTÂNCIAS ──
    if ($acao === 'listar_inst') {
        if (empty($api_base)||empty($tok)) { echo json_encode(['ok'=>false,'instances'=>[]]); exit; }
        $r = curlEvo($api_base.'/instance/fetchInstances', $tok, 'GET', null, 15);
        $d = json_decode($r['body'], true);
        $lista = [];
        if (is_array($d)) foreach ($d as $item) {
            $n = $item['instance']['instanceName'] ?? $item['instanceName'] ?? $item['name'] ?? '';
            $s = $item['instance']['connectionStatus'] ?? $item['connectionStatus'] ?? $item['state'] ?? 'unknown';
            if ($n) $lista[] = ['name'=>$n,'state'=>$s];
        }
        echo json_encode(['ok'=>true,'instances'=>$lista]);
        exit;
    }
...
// ══════════════════════════════════════════════════════
// INCLUDES DA PÁGINA — SÓ CHEGAM AQUI SE NÃO FOR AJAX
// ══════════════════════════════════════════════════════
...
if (isset($_POST['adicionar'])) {
    ...
    $func=esc_s($_POST['funcao']??''); $atv=esc_s($_POST['ativo']??'ativada'); $hora=esc_s($_POST['add_hora']??'08:00');
    $chk=$conn->query("SELECT id FROM mensagens WHERE funcao='$func' AND byid='$byid'");
    if ($chk&&$chk->num_rows>0){$msg='⚠️ Evento já tem mensagem!';$tipo='err';}
...
if (isset($_POST['editar'])) {
    ...
    $func=esc_s($_POST['edit_funcao']??''); $atv=esc_s($_POST['edit_ativo']??'ativada'); $hora=esc_s($_POST['edit_hora']??'08:00');
...
<title>WhatsApp Evolution API — Admin</title>
...
.card-header.api{background:linear-gradient(135deg,#3b82f6,#2563eb);}
...
.btn-gray{background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.12);}
...
.input-eye .fc{padding-right:42px;}
.eye-btn{position:absolute;right:12px;top:50%;transform:translateY(-50%);background:none;border:none;color:rgba(255,255,255,.4);cursor:pointer;font-size:18px;}
...
.inst-actions{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-top:8px;}
...
.vari{background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.08);border-radius:8px;padding:6px 12px;cursor:pointer;}
...
@media(max-width:768px){
    ...
    .inst-actions{grid-template-columns:1fr;}
...
<!-- Stats Card - mesmo estilo do revendedor -->
...
<div class="modern-card">
    <div class="card-header api">
        ...
        <div><div class="header-title">Configuração Global</div><div class="header-subtitle">URL e Token usados por todos os revendedores</div></div>
        <div class="header-right">
            <span class="sb <?php echo $api_ok?'ok':'warn'; ?>">
                <i class='bx bx-<?php echo $api_ok?'check-circle':'time'; ?>'></i>
                <?php echo $api_ok?'Configurada':'Não configurada'; ?>
            </span>
        </div>
    </div>
    <div class="card-body">
        <?php if ($api_ok): ?>
        <div class="nota" style="background:rgba(59,130,246,.1);border-left-color:#3b82f6;margin-bottom:16px;">
            <small>🔌 API ativa: <b><?php echo htmlspecialchars($api_base); ?></b> — todos os revendedores usam este servidor.</small>
        </div>
        <?php endif; ?>
        <div id="api-fb"></div>
        <div class="fg">
            <label><i class='bx bx-link' style="color:#38bdf8;"></i> URL da Evolution API</label>
            <input class="fc" type="text" id="evo_apiurl" value="<?php echo htmlspecialchars($api_base); ?>" placeholder="http://seu-servidor:8080">
        </div>
        <div class="fg">
            <label><i class='bx bx-key' style="color:#fbbf24;"></i> Token (apikey)</label>
            <div class="input-eye">
                <input class="fc" type="password" id="evo_token" value="<?php echo htmlspecialchars($tok_admin); ?>" placeholder="AUTHENTICATION_API_KEY do .env">
                <button type="button" class="eye-btn" onclick="toggleSenha()"><i class='bx bx-show' id="eye-icon"></i></button>
            </div>
        </div>
        <div class="nota"><small>💡 Encontre o Token no <code>.env</code> da Evolution API em <code>AUTHENTICATION_API_KEY</code>. Após salvar, todos os revendedores usam automaticamente.</small></div>
        <div class="btn-row" style="display:flex;gap:10px;margin-top:10px;">
            <button class="btn btn-cyan" onclick="testarApi()"><i class='bx bx-wifi'></i>Testar Conexão</button>
            <button class="btn btn-blue" onclick="salvarApi()"><i class='bx bx-save'></i>Salvar API Global</button>
        </div>
    </div>
</div>

<!-- CARD MINHA INSTÂNCIA -->
<div class="modern-card" <?php echo !$api_ok?'style="opacity:.5;pointer-events:none;"':''; ?>>
    <div class="card-header wpp">
        ...
        <div><div class="header-title">Minha Instância</div><div class="header-subtitle">Conexão do admin com o WhatsApp</div></div>
        <div class="header-right">
            <?php if ($wpp_online): ?>
            ...
            <?php endif; ?>
            <button class="btn btn-wpp btn-sm" onclick="abrirModal('mInst')"><i class='bx bx-plus'></i><?php echo $inst_ok?'Alterar':'Configurar'; ?></button>
        </div>
    </div>
    <div class="card-body">
        <?php if (!$api_ok): ?>
        <div class="fb err" style="margin:0;"><i class='bx bx-error-circle'></i>Configure a API Global primeiro.</div>
        <?php elseif ($inst_ok): ?>
        <div class="inst-box <?php echo $wpp_online?'on':'off'; ?>">
            ...
            <div class="inst-actions">
                <button class="btn btn-wpp" onclick="abrirQR()"><i class='bx bx-<?php echo $wpp_online?'refresh':'plug'; ?>'></i><?php echo $wpp_online?'Reconectar':'Conectar'; ?></button>
                <button class="btn btn-warn" onclick="abrirModal('mTestar')"><i class='bx bx-send'></i>Testar</button>
                <button class="btn btn-gray" onclick="listarInstancias()"><i class='bx bx-list-ul'></i>Instâncias</button>
                <button class="btn btn-danger" onclick="confirmarLogout()"><i class='bx bx-log-out'></i>Desconectar</button>
            </div>
        </div>
        <?php else: ?>
        <div class="te">
            ...
            <button class="btn btn-wpp" style="margin-top:12px;" onclick="abrirModal('mInst')"><i class='bx bx-plus'></i>Configurar agora</button>
        </div>
        <?php endif; ?>
    </div>
</div>
...
<div class="modern-card" <?php echo !$inst_ok?'style="opacity:.5;pointer-events:none;"':''; ?>>
    <div class="card-header msg">
        ...
        <div><div class="header-title">Mensagens</div><div class="header-subtitle">Disparadas nos eventos do sistema</div></div>
        <div class="header-right">
            <button class="btn btn-purple btn-sm" onclick="abrirModal('mAdd')"><i class='bx bx-plus'></i>Adicionar</button>
        </div>
    </div>
    <div class="card-body" style="padding:0;overflow-x:auto;">
        <table class="tbl">
            <thead>
                <tr><th>Mensagem</th><th>Evento</th><th>Horário</th><th>Status</th><th></th>
                </thead>
            <tbody>
            ...
            ?>
                <td style="max-width:180px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;"><?php echo htmlspecialchars(mb_substr($rm['mensagem'],0,45)).(mb_strlen($rm['mensagem'])>45?'…':''); ?>}]
...
<div class="modal-overlay" id="mInst"><div class="modal-container"><div class="modal-content">
    ...
    <form method="POST"><div class="modal-body">
        <div class="nota warn"><small>⚠️ Crie apenas uma vez. Se já existir na API, será vinculada automaticamente.</small></div>
        <div class="fg"><label><i class='bx bx-cube'></i> Nome da Instância</label><input class="fc" type="text" name="instancia" id="inst_nome" value="<?php echo htmlspecialchars($inst_atual); ?>" placeholder="ex: admin_principal"></div>
        ...
        <button type="button" class="btn btn-cyan" onclick="executarCriar()"><i class='bx bx-play'></i>Criar</button>
...
<div class="modal-overlay" id="mQR"><div class="modal-container"><div class="modal-content">
    <div class="modal-header success"><h5><i class='bx bx-qr-scan'></i>Conectar WhatsApp</h5><button class="modal-close" onclick="fecharModal('mQR');pararVerif();"><i class='bx bx-x'></i></button></div>
    <div class="modal-body"><div id="qr-area"><div class="qm"><span class="spin"></span> Gerando QR...</div></div><div class="nota" style="margin-top:12px;"><small>📱 WhatsApp → Configurações → Dispositivos Vinculados → Vincular dispositivo</small></div></div>
    <div class="modal-footer"><button class="btn btn-gray" onclick="fecharModal('mQR');pararVerif();"><i class='bx bx-x'></i>Fechar</button><button class="btn btn-cyan" onclick="carregarQR()"><i class='bx bx-refresh'></i>Novo QR</button><button class="btn btn-success" onclick="verificarStatus()"><i class='bx bx-check-circle'></i>Verificar</button></div>
</div></div></div>

<div class="modal-overlay" id="mInstApi"><div class="modal-container wide"><div class="modal-content">
    <div class="modal-header primary"><h5><i class='bx bx-list-ul'></i>Instâncias na API</h5><button class="modal-close" onclick="fecharModal('mInstApi')"><i class='bx bx-x'></i></button></div>
    <div class="modal-body"><div class="nota warn"><small>⚠️ Visualize e apague instâncias desnecessárias de revendedores.</small></div><div id="inst-api-res"><div class="te"><span class="spin"></span> Carregando...</div></div></div>
    <div class="modal-footer"><button class="btn btn-gray" onclick="fecharModal('mInstApi')"><i class='bx bx-x'></i>Fechar</button><button class="btn btn-blue" onclick="listarInstancias()"><i class='bx bx-refresh'></i>Atualizar</button></div>
</div></div></div>
...
<div class="modal-overlay" id="mTestar"><div class="modal-container"><div class="modal-content">
    ...
    <div class="modal-footer"><button class="btn btn-gray" onclick="fecharModal('mTestar')"><i class='bx bx-x'></i>Fechar</button><button class="btn btn-warn" onclick="executarTeste()"><i class='bx bx-send'></i>Enviar</button></div>
</div></div></div>
...
<div class="modal-overlay" id="mEditar"><div class="modal-container wide"><div class="modal-content">
    ...
    <form method="POST"><input type="hidden" name="edit_id" id="edit_id"><div class="modal-body">
        ...
        <div class="grid2">
            <div class="fg"><label><i class='bx bx-calendar-event'></i>Evento</label><select class="fc" name="edit_funcao" id="edit_func" onchange="toggleHoraEdit()"><?php foreach($funcoes_labels as $k=>$fl):?><option value="<?php echo $k;?>"><?php echo $fl['label'];?></option><?php endforeach;?></select></div>
            ...
            <div class="fg"><label><i class='bx bx-toggle-left'></i>Status</label><select class="fc" name="edit_ativo" id="edit_ativo"><option value="ativada">Ativada</option><option value="desativado">Desativada</option></select></div>
        </div>
    </div><div class="modal-footer">
...
<div class="modal-overlay" id="mAdd"><div class="modal-container wide"><div class="modal-content">
    ...
    <form method="POST"><div class="modal-body">
        <div class="fg"><label><i class='bx bx-calendar-event'></i>Evento</label><select class="fc" name="funcao" id="add_func" onchange="preencherTpl();toggleHoraAdd();"><?php foreach($funcoes_labels as $k=>$fl):?><option value="<?php echo $k;?>"><?php echo $fl['label'];?></option><?php endforeach;?></select></div>
        ...
        <div class="fg"><label><i class='bx bx-toggle-left'></i>Status</label><select class="fc" name="ativo"><option value="ativada">Ativada</option><option value="desativado">Desativada</option></select></div>
    </div><div class="modal-footer">
...
function salvarApi(){
    var url=document.getElementById('evo_apiurl').value.trim();
    var tok=document.getElementById('evo_token').value.trim();
    var fb=document.getElementById('api-fb');
    if(!url||!tok){fb.innerHTML='<div class="fb err"><i class="bx bx-error-circle"></i> Preencha URL e Token!</div>';return;}
    fb.innerHTML='<div class="fb info"><span class="spin"></span> Salvando...</div>';
    var fd=new FormData();fd.append('evo_apiurl',url);fd.append('evo_token',tok);
    fetch(PHP_URL+'?ajax=salvar_api',{method:'POST',body:fd,credentials:'same-origin'})
        .then(r=>r.json()).then(d=>{
            fb.innerHTML='<div class="fb '+(d.ok?'ok':'err')+'"><i class="bx bx-'+(d.ok?'check-circle':'error-circle')+'"></i> '+d.msg+'</div>';
            if(d.ok) setTimeout(()=>location.reload(),1500);
        }).catch(e=>{fb.innerHTML='<div class="fb err"><i class="bx bx-error-circle"></i> '+e.message+'</div>';});
}
...
function testarApi(){
    ...
    var fb=document.getElementById('api-fb');
    if(!url||!tok){fb.innerHTML='<div class="fb err"><i class="bx bx-error-circle"></i> Preencha URL e Token!</div>';return;}
    fb.innerHTML='<div class="fb info"><span class="spin"></span> Testando...</div>';
    var fd=new FormData();fd.append('evo_apiurl',url);fd.append('evo_token',tok);
    fetch(PHP_URL+'?ajax=testar_api',{method:'POST',body:fd,credentials:'same-origin'})
        .then(r=>r.json()).then(d=>{
            fb.innerHTML='<div class="fb '+(d.ok?'ok':'err')+'"><i class="bx bx-'+(d.ok?'check-circle':'error-circle')+'"></i> '+d.msg+'</div>';
        }).catch(e=>{fb.innerHTML='<div class="fb err"><i class="bx bx-error-circle"></i> '+e.message+'</div>';});
}
...
function toggleSenha(){
    var i=document.getElementById('evo_token'),c=document.getElementById('eye-icon');
    i.type=i.type==='password'?'text':'password';
    c.className=i.type==='password'?'bx bx-show':'bx bx-hide';
}
...
function executarCriar(){
    ...
    res.innerHTML='<div class="fb info"><span class="spin"></span> Criando...</div>';
    var fd=new FormData();fd.append('instancia',nome);
    fetch(PHP_URL+'?ajax=criar',{method:'POST',body:fd,credentials:'same-origin'})
        .then(r=>r.json()).then(d=>{
            if(d.ok){res.innerHTML='<div class="fb ok"><i class="bx bx-check-circle"></i> '+d.msg+'</div>';setTimeout(()=>location.reload(),1500);}
            else{res.innerHTML='<div class="fb err"><i class="bx bx-error-circle"></i> '+(d.erro||JSON.stringify(d))+'</div>';}
...
function carregarQR(){
    ...
    fetch(PHP_URL+'?ajax=qr',{credentials:'same-origin'}).then(r=>r.json()).then(d=>{
        if(d.state==='open'){document.getElementById('qr-area').innerHTML='<div class="qok"><i class="bx bx-check-circle"></i> Já conectado!</div>';setTimeout(()=>location.reload(),1500);return;}
        var b64=d.base64||d.qrcode||null;
        if(b64){document.getElementById('qr-area').innerHTML='<img src="'+b64+'" alt="QR"><div class="qm">📱 Escaneie</div>';iniciarVerif();}
        else if(d.erro){document.getElementById('qr-area').innerHTML='<div class="qm" style="color:#f87171;"><i class="bx bx-error-circle"></i> '+d.erro+'</div>';}
        else{document.getElementById('qr-area').innerHTML='<div class="qm" style="color:#f87171;">QR não disponível.</div>';}
...
function verificarStatus(){
    fetch(PHP_URL+'?ajax=status',{credentials:'same-origin'}).then(r=>r.json()).then(d=>{
        var st=d.state||(d.instance&&d.instance.state)||'';
        if(st==='open'){pararVerif();document.getElementById('qr-area').innerHTML='<div class="qok"><i class="bx bx-check-circle"></i> Conectado!</div>';setTimeout(()=>location.reload(),1500);}
    });
}

function confirmarLogout(){
    if(!confirm('Desconectar o WhatsApp?'))return;
    fetch(PHP_URL+'?ajax=logout',{method:'POST',credentials:'same-origin'}).then(()=>location.reload());
}

function listarInstancias(){
    document.getElementById('inst-api-res').innerHTML='<div class="te"><span class="spin"></span> Carregando...</div>';
    abrirModal('mInstApi');
    fetch(PHP_URL+'?ajax=listar_inst',{credentials:'same-origin'}).then(r=>r.json()).then(d=>{
        if(!d.ok||!d.instances||d.instances.length===0){document.getElementById('inst-api-res').innerHTML='<div class="te">Nenhuma instância encontrada.</div>';return;}
        var minha='<?php echo addslashes($inst_atual); ?>';
        var html='<div style="display:flex;flex-direction:column;gap:8px;">';
        d.instances.forEach(function(inst){
            var isMine=inst.name.toLowerCase()===minha.toLowerCase();
            var isOn=inst.state==='open';
            html+='<div style="display:flex;align-items:center;justify-content:space-between;background:rgba(255,255,255,'+(isMine?'.07':'.03')+');border:1px solid rgba(255,255,255,'+(isMine?'.2':'.07')+');border-radius:10px;padding:10px 14px;">';
            html+='<div style="display:flex;align-items:center;gap:8px;"><span class="pulse-dot '+(isOn?'on':'off')+'"></span>'+inst.name;
            if(isMine) html+=' <span class="sb ok" style="font-size:9px;">minha</span>';
            html+=' <span class="sb '+(isOn?'ok':'off')+'" style="font-size:9px;">'+inst.state+'</span></div>';
            html+='<button class="btn btn-danger btn-sm" onclick="deletarInstApi(\''+inst.name+'\',this)"><i class="bx bx-trash"></i>Apagar</button>';
            html+='</div>';
        });
        html+='</div>';
        document.getElementById('inst-api-res').innerHTML=html;
    }).catch(e=>{document.getElementById('inst-api-res').innerHTML='<div class="te" style="color:#f87171;">Erro: '+e.message+'</div>';});
}

function deletarInstApi(nome,btn){
    if(!confirm('Apagar "'+nome+'"? Irreversível!'))return;
    btn.disabled=true;btn.innerHTML='<span class="spin"></span>';
    var fd=new FormData();fd.append('target_byid','0');fd.append('nome',nome);
    fetch(PHP_URL+'?ajax=deletar_inst',{method:'POST',body:fd,credentials:'same-origin'})
        .then(r=>r.json()).then(d=>{
            if(d.ok) btn.closest('div[style]').remove();
            else{alert(d.msg||'Erro');btn.disabled=false;btn.innerHTML='<i class="bx bx-trash"></i>Apagar';}
        }).catch(e=>{alert(e.message);btn.disabled=false;btn.innerHTML='<i class="bx bx-trash"></i>Apagar';});
}
...
function editarMsg(id){
    fetch(PHP_URL+'?ajax=get_msg&id='+id,{credentials:'same-origin'}).then(r=>r.json()).then(d=>{
        if(!d.id)return;
        ...
        toggleHoraEdit();abrirModal('mEditar');
    });
}
