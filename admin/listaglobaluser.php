...
$where = !empty($search) ? "WHERE login LIKE '%$search%'" : "";

$sql_total = "SELECT COUNT(*) as total FROM ssh_accounts $where";
...
$sql = "SELECT * FROM ssh_accounts $where ORDER BY FIELD(status,'Online','Offline'), expira ASC LIMIT $limite_por_pagina OFFSET $offset";
$result = $conn->query($sql);

// Stats
$total_online = 0; $r = $conn->query("SELECT COUNT(*) as t FROM ssh_accounts WHERE status='Online'"); if($r){$total_online=$r->fetch_assoc()['t'];}
$total_vencidos = 0; $r = $conn->query("SELECT COUNT(*) as t FROM ssh_accounts WHERE expira < NOW()"); if($r){$total_vencidos=$r->fetch_assoc()['t'];}
$total_suspensos = 0; $r = $conn->query("SELECT COUNT(*) as t FROM ssh_accounts WHERE mainid='Suspenso'"); if($r){$total_suspensos=$r->fetch_assoc()['t'];}
$total_limite = 0; $r = $conn->query("SELECT COUNT(*) as t FROM ssh_accounts WHERE mainid='Limite Ultrapassado'"); if($r){$total_limite=$r->fetch_assoc()['t'];}
...
<title>Lista Global de Usuários</title>
...
.limit-selector{display:flex;align-items:center;gap:8px;background:rgba(255,255,255,0.04);padding:5px 12px;border-radius:10px;border:1px solid rgba(255,255,255,0.08);}
.limit-selector label{font-size:11px;color:rgba(255,255,255,0.6);display:flex;align-items:center;gap:4px;}
.limit-selector select{background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.15);border-radius:8px;padding:4px 8px;color:#fff;font-size:11px;cursor:pointer;}
...
/* Dias select */
.dias-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:6px;margin-top:8px;}
.dia-opt{background:rgba(255,255,255,.06);border:1.5px solid rgba(255,255,255,.08);border-radius:8px;padding:7px 4px;text-align:center;cursor:pointer;transition:all .2s;font-size:11px;font-weight:600;color:rgba(255,255,255,.6);}
...
<div class="content-wrapper">
    ...
    <div class="stats-card">
        ...
        <div class="stats-card-content">
            <div class="stats-card-title">Lista Global de Usuários</div>
            <div class="stats-card-value"><?php echo $total_registros; ?></div>
            <div class="stats-card-subtitle">todos os usuários do sistema</div>
        </div>
    ...
    <div class="mini-stats">
        ...
        <div class="mini-stat"><div class="mini-stat-val" style="color:#fb923c;"><?php echo $total_limite; ?></div><div class="mini-stat-lbl">Limite</div></div>
    </div>
    ...
    <div class="modern-card">
        <div class="card-header-custom blue">
            ...
            <div><div class="header-title">Filtros de Busca</div><div class="header-subtitle">Busca global em todos os revendedores</div></div>
        </div>
            ...
            <div class="filter-group">
                ...
                <div class="filter-item">
                    ...
                    <select class="form-control" id="statusFilter" onchange="filtrarUsuarios()">
                        ...
                        <option value="limite">⚠️ Limite</option>
    ...
    if ($result && $result->num_rows > 0):
        while ($row = $result->fetch_assoc()):
            ...
            $limite = $row['limite']; $expira = $row['expira']; $status = $row['status'];
            $categoria = $row['categoriaid']; $suspenso = $row['mainid'];
            $notas = $row['lastview']; $uuid = $row['uuid']; $byid_ssh = $row['byid'];
            ...
            $is_suspenso = ($suspenso == 'Suspenso');
            $is_limite = ($suspenso == 'Limite Ultrapassado');
            if ($is_suspenso) { $st_class='status-suspended'; $st_icon='bx-lock'; $st_text='Suspenso'; $st_data='suspenso'; }
            elseif ($is_limite) { $st_class='status-limit'; $st_icon='bx-error'; $st_text='Limite'; $st_data='limite'; }
            elseif ($status === 'Online') { $st_class='status-online'; $st_icon='bx-wifi'; $st_text='Online'; $st_data='online'; }
            else { $st_class='status-offline'; $st_icon='bx-power-off'; $st_text='Offline'; $st_data='offline'; }
            ...
            $rev_login = 'N/A';
            if ($byid_ssh) { $r_rev = $conn->query("SELECT login FROM accounts WHERE id='$byid_ssh'"); if ($r_rev && $r_rev->num_rows > 0) $rev_login = $r_rev->fetch_assoc()['login']; }

            $limite_html = ($usando > 0) ? '<span class="'.($usando >= $limite ? 'danger' : '').'">'.$usando.'/'.$limite.'</span>' : $limite;
    ?>
    <div class="user-card" data-login="<?php echo strtolower(htmlspecialchars($login)); ?>" data-status="<?php echo $st_data; ?>" data-expirado="<?php echo $dias_rest<0?'expirado':''; ?>" data-id="<?php echo $id; ?>" data-usuario="<?php echo htmlspecialchars($login); ?>" data-senha="<?php echo htmlspecialchars($senha); ?>" data-limite="<?php echo $limite; ?>" data-expira="<?php echo $expira_fmt; ?>" data-revendedor="<?php echo htmlspecialchars($rev_login); ?>">
        <div class="user-header">
            ...
            <button class="btn-copy-card" onclick="copiar(this,event)"><i class='bx bx-copy'></i> Copiar</button>
        </div>
        <div class="user-body">
            <div class="status-row">
                <div class="status-item-card">
                    <div class="status-icon"><i class='bx <?php echo $st_icon; ?>'></i></div>
                    <div class="status-content"><div class="status-label">STATUS</div><span class="status-badge <?php echo $st_class; ?>"><?php echo $st_text; ?></span></div>
                </div>
                <div class="status-item-card">
            ...
            <div class="info-grid">
                ...
                <div class="info-row"><div class="info-icon"><i class='bx bx-store' style="color:#f472b6;"></i></div><div class="info-content"><div class="info-label">REVENDEDOR</div><div class="info-value"><?php echo htmlspecialchars($rev_login); ?></div></div></div>
                <div class="info-row"><div class="info-icon"><i class='bx bx-group' style="color:#34d399;"></i></div><div class="info-content"><div class="info-label">LIMITE</div><div class="info-value"><?php echo $limite_html; ?></div></div></div>
                ...
                <?php if(!empty($notas)):?>
                <div class="info-row" style="grid-column:1/-1;"><div class="info-icon"><i class='bx bx-note' style="color:#a78bfa;"></i></div><div class="info-content"><div class="info-label">NOTAS</div><div class="info-value"><?php echo htmlspecialchars($notas); ?></div></div></div>
                <?php endif;?>
            </div>
            <div class="user-actions">
                ...
                <button class="action-btn btn-renew" onclick="renovar(<?php echo $id; ?>)"><i class='bx bx-refresh'></i> Renovar</button>
                <?php if($is_suspenso):?>
    ...
    <?php endwhile; else: ?>
    <div class="empty-state"><i class='bx bx-user-x'></i><h3>Nenhum usuário encontrado</h3><p>Tente ajustar os filtros.</p></div>
    <?php endif; ?>
    </div>
    ...
    <?php endif;?>
    <div class="pagination-info">Mostrando <?php echo min($offset+1,$total_registros);?>–<?php echo min($offset+$limite_por_pagina,$total_registros);?> de <?php echo $total_registros;?> usuários — Página <?php echo $pagina_atual;?>/<?php echo $total_paginas;?></div>
...
<div class="modal-container"><div class="modal-content-custom">
    ...
    <div class="modal-body-custom">
        ...
        <div class="modal-user-info" id="renovarInfo"></div>
        <div class="dias-grid">
            <div class="dia-opt" onclick="setDias(7)">7d</div><div class="dia-opt" onclick="setDias(15)">15d</div>
            <div class="dia-opt active" onclick="setDias(30)">30d</div><div class="dia-opt" onclick="setDias(60)">60d</div>
            <div class="dia-opt" onclick="setDias(90)">90d</div><div class="dia-opt" onclick="setDias(180)">180d</div>
            <div class="dia-opt" onclick="setDias(365)">1 ano</div>
        </div>
        <input type="hidden" id="renovarDias" value="30">
    </div>
...
<div class="modal-container"><div class="modal-content-custom">
    ...
    <div class="modal-body-custom">
        ...
        <p style="text-align:center;font-size:11px;color:rgba(255,255,255,.4);margin-top:6px;">O usuário ficará suspenso até ser reativado.</p>
    </div>
    <div class="modal-footer-custom">
...
<div class="modal-container"><div class="modal-content-custom">
    ...
    <div class="modal-body-custom">
        ...
        <p style="text-align:center;font-size:11px;color:rgba(255,255,255,.4);margin-top:6px;">Todos os dispositivos serão desconectados.</p>
    </div>
    <div class="modal-footer-custom">
...
function userInfoHTML(card){
    return '<div class="modal-user-row"><i class="bx bx-user" style="color:#818cf8;"></i> <span>Usuário:</span> <strong>'+(card.dataset.usuario||'')+'</strong></div>'
        ...
        +'<div class="modal-user-row"><i class="bx bx-calendar" style="color:#fbbf24;"></i> <span>Validade:</span> <strong>'+(card.dataset.expira||'')+'</strong></div>'
        +'<div class="modal-user-row"><i class="bx bx-store" style="color:#f472b6;"></i> <span>Revendedor:</span> <strong>'+(card.dataset.revendedor||'N/A')+'</strong></div>';
}
...
function filtrarUsuarios(){
    ...
    document.querySelectorAll('.user-card').forEach(function(c){
        ...
        else if(status==='limite')ms=st==='limite';
...
function setDias(d){
    ...
    document.querySelectorAll('.dia-opt').forEach(function(o){o.classList.remove('active');var v=o.textContent.trim();var val=v==='1 ano'?365:parseInt(v);if(val===d)o.classList.add('active');});
}

// ══════════════════════════════════════════
// RENOVAR → renovardias.php?id=X via GET
// ══════════════════════════════════════════
function renovar(id){
    _id=id;_card=getCard(id);
    document.getElementById('renovarInfo').innerHTML=userInfoHTML(_card);
    document.getElementById('renovarDias').value=30;setDias(30);
    abrirModal('modalRenovar');
}
function confirmarRenovar(){
    var dias=parseInt(document.getElementById('renovarDias').value)||30;
    fecharModal('modalRenovar');
    document.getElementById('processandoTexto').textContent='Renovando '+(_card?_card.dataset.usuario:'')+'...';
    abrirModal('modalProcessando');
    ajaxGET('renovardias.php?id='+_id+'&dias='+dias,function(resp){
        if(respOk(resp,['sucesso','renovado'])){
            toast('Renovado com sucesso! (+'+dias+' dias)','ok');
            setTimeout(function(){location.reload();},1500);
        }else{
            // Tenta extrair mensagem JSON
            try{var j=JSON.parse(resp);toast(j.mensagem||'Erro ao renovar!','err');}
...
// ══════════════════════════════════════════
// SUSPENDER → suspender.php?id=X via GET
// ══════════════════════════════════════════
...
function confirmarSuspender(){
    ...
    ajaxGET('suspender.php?id='+_id,function(resp){
        if(respOk(resp,['suspenso','sucesso'])){
            toast('Usuário suspenso com sucesso!','ok');
            setTimeout(function(){location.reload();},1500);
        }else if(resp.toLowerCase().indexOf('erro no servidor')!==-1){
            toast('Erro no servidor! Verifique se está online.','err');
        }else{
            toast('Erro ao suspender!','err');
        }
    });
}

// ══════════════════════════════════════════
// REATIVAR → reativar.php?id=X via GET
// ══════════════════════════════════════════
...
function confirmarReativar(){
    ...
    ajaxGET('reativar.php?id='+_id,function(resp){
        if(respOk(resp,['reativado','sucesso'])){
            toast('Usuário reativado com sucesso!','ok');
            setTimeout(function(){location.reload();},1500);
        }else{
...
// ══════════════════════════════════════════
// EXCLUIR → excluiruser.php?id=X via GET
// ══════════════════════════════════════════
function excluir(id){
    _id=id;_card=getCard(id);
...
function confirmarExcluir(){
    ...
    ajaxGET('excluiruser.php?id='+_id,function(resp){
        if(respOk(resp,['excluido','sucesso'])){
...
// ══════════════════════════════════════════
// DEVICE → deviceid.php?id=X via GET
// ══════════════════════════════════════════
...
function confirmarDevice(){
    ...
    ajaxGET('deviceid.php?id='+_id,function(resp){
        if(respOk(resp,['deletado','sucesso','limpo'])){
            toast('Device ID limpo com sucesso!','ok');
            setTimeout(function(){location.reload();},1500);
        }else{
            toast('Erro ao limpar Device ID!','err');
        }
    });
}
</script>
