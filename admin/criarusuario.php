...
$modal_usuario          = '';
$modal_senha            = '';
$modal_validade         = date('Y-m-d H:i:s');
$modal_limite           = '';
$modal_uuid             = '';
$modal_v2ray            = '';
$modal_valormensal      = '0';
...
if (isset($_POST['criaruser'])) {
    ...
    $validadefin      = anti_sql($_POST['validadefin'] ?? '30');
    $limitefin        = anti_sql($_POST['limitefin']   ?? '1');
    // ✅ CORREÇÃO PRINCIPAL: categoria NÃO passa pelo anti_sql
    ...
    elseif  (strlen($usuariofin) < 5)                  $error_message = 'Usuário deve ter no mínimo 5 caracteres!';
    elseif  (strlen($usuariofin) > 10)                 $error_message = 'Usuário deve ter no máximo 10 caracteres!';
    elseif  (strlen($senhafin) < 5)                    $error_message = 'Senha deve ter no mínimo 5 caracteres!';
    elseif  (strlen($senhafin) > 10)                   $error_message = 'Senha deve ter no máximo 10 caracteres!';
    ...
    if (!$error_message) {
        $chk = $conn->query("SELECT id FROM ssh_accounts WHERE login='$usuariofin'");
        if ($chk && $chk->num_rows > 0) $error_message = 'Usuário já existe!';
    }
    ...
    if ($error_message) {
        ...
        // ✅ CORREÇÃO: usa $res_srv dedicado para não conflitar
        ...
        if (!$sucess) {
            $error_message    = 'Erro ao criar usuário! Nenhum servidor disponível.' . (!empty($failed_servers) ? ' Offline: ' . implode(', ', $failed_servers) : '');
            $show_error_modal = true;
        } else {
            mysqli_query($conn, "INSERT INTO logs (revenda, byid, validade, texto, userid)
                VALUES ('{$_SESSION['login']}', '{$_SESSION['byid']}', '$datahoje',
                'Criou um Usuario $usuariofin de $validadefin Dias', '{$_SESSION['iduser']}')");

            $expira = date('Y-m-d H:i:s', strtotime("+$validadefin days"));
            ...
            if (!empty($whatsapp_destino)) {
                $sql_msg = "SELECT mensagem FROM mensagens WHERE funcao='criarusuario' AND (byid='{$_SESSION['iduser']}' OR byid='') AND ativo='ativada' ORDER BY byid DESC LIMIT 1";
                $r_msg   = mysqli_query($conn, $sql_msg);
                if ($r_msg && mysqli_num_rows($r_msg) > 0) {
                    $msg_template = mysqli_fetch_assoc($r_msg)['mensagem'];
                } else {
                    $msg_template = "🎉 *Usuário Criado!*\n\n👤 Usuário: {usuario}\n🔑 Senha: {senha}\n📅 Validade: {validade}\n👥 Limite: {limite} conexões\n\n🌐 https://{dominio}/";
                }
                $msg_final = str_replace(
                    ['{usuario}','{login}','{senha}','{validade}','{limite}','{dominio}'],
                    [$usuariofin,$usuariofin,$senhafin,date('d/m/Y',strtotime($expira)),$limitefin,$_SERVER['HTTP_HOST']],
                    $msg_template
                );
            ...
            $mensagem_modal = '';
            $sql_mm = "SELECT mensagem FROM mensagens_modal WHERE funcao='criarusuario' AND byid='{$_SESSION['iduser']}' AND ativo='ativada' LIMIT 1";
...
if (function_exists('getCSSVariables')) {
    ...
    echo ':root{--primaria:#10b981;--secundaria:#C850C0;--fundo:#0f172a;--fundo_claro:#1e293b;--texto:#ffffff;--sucesso:#10b981;--erro:#dc2626;--aviso:#f59e0b;--info:#3b82f6;}';
}
?>
...
<div class="stats-card">
    ...
    <div class="stats-card-content">
        ...
        <div class="stats-card-subtitle">Adicione um novo cliente ao sistema</div>
    </div>
            ...
            <div class="form-grid">
                ...
                <div class="form-field">
                    ...
                    <input type="text" class="form-control" name="usuariofin" id="usuariofin"
                           placeholder="ex: usuario123" minlength="5" maxlength="10" required>
                </div>
                ...
                <div class="form-field">
                    ...
                    <input type="text" class="form-control" name="senhafin" id="senhafin"
                           placeholder="ex: senha123" minlength="5" maxlength="10" required>
                </div>
...
<div class="modal-container wide"><div class="modal-content">
    ...
    <div class="modal-body">
        ...
        <!-- ✅ BOTÃO WPP MANUAL (caso automático não tenha enviado) -->
        <?php if ($show_modal && !empty($modal_whatsapp_destino) && !$wpp_enviado): ?>
        <button class="btn-wpp-manual" id="btnWppManual"
            onclick="enviarWppManual('<?php echo htmlspecialchars($modal_whatsapp_destino); ?>',
                '<?php echo addslashes($modal_usuario); ?>',
                '<?php echo addslashes($modal_senha); ?>',
                '<?php echo $show_modal ? date("d/m/Y", strtotime($modal_validade)) : ""; ?>',
...
function enviarWppManual(numero, usuario, senha, validade, limite){
    ...
    fetch('criarusuarios.php?ajax=enviar_wpp', { method:'POST', body: fd })
